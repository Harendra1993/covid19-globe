...
var Globe = /** @class */ (function () {
  function Globe(canvas, tooltipDiv) {
    ...
    scene.on("mousemove", function (event) {
      ...
      if (_this.activeMarker) {
        ...
        _this.callbacks.onMouseOutMarker(
          _this.activeMarker,
          _this.activeMarkerObject,
  ...
  Globe.prototype.updateMarkers = function (markers, markerOptions) {
    ...
    var sizeScale = scaleLinear()
      .domain([
        minValue(markers, function (marker) {
          return marker.value
        }),
    ...
    var markerObjectNames = new Set(
      this.markerObjects.children.map(function (markerObject) {
        ...
        if (renderer !== undefined) {
          ...
          tween(
            ...
            function () {
              switch (type) {
                case MarkerType.Bar:
                  mesh_1.geometry = new BoxGeometry(
                    unitRadius,
                    unitRadius,
                    from_4.size
                  )
                ...
                case MarkerType.Dot:
                default:
                  mesh_1.geometry = new SphereGeometry(
                    from_4.size,
                    MARKER_SEGMENTS,
                    MARKER_SEGMENTS
                  )
    ...
    var markerObjectsToRemove = this.markerObjects.children.filter(function (
      markerObject
    ) {
  ...
  Globe.prototype.updateOptions = function (options, key) {
    ...
    this.options = __assign(
      ...
      ((_a = {}),
      (_a[key] = __assign(__assign({}, defaultOptions[key]), options)),
      _a)
    )
  }
...
function ReactGlobe(_a) {
  ...
  return React.createElement(
    "div",
    { ref: mountRef, style: { height: "100%", width: "100%" } },
